# Pryti: See What a Pull Request *Actually* Changes — On Every Repo You Own

*Git shows you what **lines** changed. Pryti shows you what they **mean**, where to look, and whether
they broke a rule. It's a semantic PR reviewer for Python web apps. This guide starts from zero —
every idea, command, and setting in plain English — then shows you how to set Pryti up **once** and
...
1. [What is Pryti (in one minute)](#1-what-is-pryti-in-one-minute)
...
4. [Install](#4-install)
5. [Three ways to use Pryti](#5-three-ways-to-use-pryti)
...
10. [Set up Pryti for all your repos (personal, team, or org)](#10-set-up-pryti-for-all-your-repos)
11. [A full worked example](#11-a-full-worked-example)
12. [Honesty rules (why Pryti never lies)](#12-honesty-rules)
13. [Quick reference card](#13-quick-reference-card)

---

## 1. What is Pryti (in one minute)
...
**Pryti reads your PR and tells you what it *means*** — in a few high-signal findings, each pointing
...
![The Pryti landing page](images/00-welcome.png)

---

## 2. The one big idea

Every other tool answers **"what changed?"** Pryti answers a second, harder question:
...
security holes live. Almost every Pryti feature is a version of comparing those two things:
...
Example: `POST /api/users/export` runs the `ExportUsers` function. Pryti reviews your PR one
endpoint at a time.

**The 6-edge lens** — for each endpoint, Pryti asks six simple questions (these are the "edges"):
...
**Severity colors** — Pryti ranks every finding by how much it should worry you:
...
**Taint** — following a piece of data as it moves through the code. If an email is "tainted," Pryti
...
Example: *"every endpoint that writes to the database first checks you're logged in."* Pryti can
**discover** these from your git history, let you **confirm** the real ones, and then **enforce**
them on every PR.

**Semantic diff** — a normal diff compares *text*. A **semantic diff** compares *meaning*: Pryti
...
That's it — Pryti has **zero dependencies** (it only uses Python's standard library). You now have a
`pryti` command. Check it:
...
Commands:
  ...
  serve      Start the Pryti web UI
  invariants Discover baseline invariants from a repository's history
  digest     Org-wide roll-up from the labels Pryti applies to PRs
```

---

## 5. Three ways to use Pryti

You can use Pryti in whichever way fits:

1. **Web UI** — run `pryti serve`, open your browser, click around. Best for **exploring** and for
   people who don't live in the terminal. → [jump to the tour](#6-the-web-ui-panel-by-panel)
2. **Command line** — `pryti review …`. Best for **scripts, quick checks, and CI**. →
   [jump to the CLI](#7-the-cli-command-by-command)
3. **GitHub Action** — Pryti runs automatically on every PR and comments on it. Best for **teams**.
...
2. **🎭 Intent vs. behavior** — the red panel. The PR called itself a *"refactor / no behavior
   change,"* but Pryti found it **added an endpoint** and **started logging an email**. When the
...
6. **The flow graph** (right) — a live picture of what that endpoint does. In the screenshot,
   ...
   parts highlighted. This is the same picture Pryti can also draw as a
   [Mermaid diagram](#feature-7--mermaid-flow-diagram) inside a GitHub comment.

### The Invariants panel — teach Pryti your rules

Click **⚖ Invariants** in the top bar:

![The invariants panel: discover, confirm, blame](images/02-invariants.png)

Pryti samples your git history and shows **candidate rules** it noticed your codebase following. In
the screenshot:
...
- Each rule has a **Confirm** button (freeze it as a rule Pryti will enforce from now on) and a
  **🩸 Blame** button (find the commit that first broke it).

This is the **moat** — over time you teach Pryti the rules *your* codebase cares about, and no
...
> `pryti serve --invariants my-rules.json`. Without that, the panel is read-only (Pryti never writes
...
### `pryti review` — the main event

Analyze a PR / branch / commit and print a review.

```bash
pryti review                       # no args: review THIS branch vs the default branch
pryti review /path/to/repo --base main --head my-feature
pryti review https://github.com/owner/repo --pr 123
```
...
| Argument | Meaning |
|----------|---------|
| `repo` | A local folder **or** a GitHub URL. **Optional** — leave it out and Pryti uses the repo you're standing in. |
...
### `pryti post` — put the review on the PR

Takes a review you already generated and posts it to a GitHub PR.

```bash
pryti post 123 pr_review.md --json pr_review.json --inline --label
```
...
| `--label` | Put one `pryti:*` **label** on the PR based on its worst finding. ([feature #6](#feature-6--auto-label)) |
...
### `pryti serve` — the web UI

```bash
pryti serve --repo /path/to/repo --port 8765
```
...
| `PRYTI_TOKEN` | Require `?token=…` on API calls — **set this on any public deployment.** |
| `PRYTI_ALLOWED_REPOS` | Comma-separated allow-list; only matching repos may be reviewed. |
| `PRYTI_BASE_PATH` | Serve under a sub-path (e.g. `/pryti`) behind a reverse proxy. |
| `PRYTI_DEFAULT_REPO` | The repo shown by default in the UI. |
| `PRYTI_INVARIANTS` | Default invariant corpus path to enforce in the UI. |
| `GITHUB_TOKEN` | For private repos / to avoid GitHub rate limits. |

To host one shared Pryti UI: run `pryti serve --no-open` behind your usual reverse proxy, set
`PRYTI_TOKEN` (so the API isn't open), and optionally `PRYTI_ALLOWED_REPOS` to fence it to just your
repos. To run Pryti across *all* your repos from one place, see
[§10 — Set up Pryti for all your repos](#10-set-up-pryti-for-all-your-repos).
...
```bash
pryti invariants /path/to/repo --snapshots 8
```
...
```bash
pryti invariants --confirm                       # interactive: press c / f / s / q per rule
pryti invariants --confirm --id auth-before-write --owner security   # scripted
```
...
```bash
pryti invariants /path/to/repo --blame auth-before-write --exact
```
...
### `pryti digest` — the org-wide roll-up for leadership
...
It works by counting labels. Each PR review applies exactly one `pryti:*` triage label
...
```
*Pryti digest — my-org*  (open PRs by risk)
...
answer to "is Pryti worth keeping on?" — because the labels feed it, keep `label: true` and
...
Pryti analyzes your code **before** and **after** the PR and reports the change in **behavior**.
...
Other tools generate this with an AI that reads the diff and sometimes makes things up. Pryti counts
real facts, so every number traces to a line.

### Feature #4 — intent vs. behavior contradiction

Your PR title/description is a **promise**. Pryti checks it against the facts.
...
When a PR bumps a library, Pryti downloads the **old** and **new** versions and compares what each
...
Turn the network scan off with `PRYTI_SCAN_DEPS=0` (or the Action's `scan_deps: false`) and you'll
always get the fast ⚠ mode.

### Feature #9 — PII taint

The star precision feature. Old tools say *"personal data and an external call are both in this
function — maybe it leaks."* Vague. Pryti **follows the data**:
...
- If the data passes through a function Pryti doesn't recognize (which *might* clean it) → it
...
1. **Discover** (`pryti invariants`) — mine history for rules your code already follows:
...
2. **Confirm** (`pryti invariants --confirm`, or the web button) — mark the ones that are real. Saved
   to a rules file. No JSON editing.

3. **Enforce** (`pryti review --invariants rules.json`) — every future PR that breaks a confirmed
   rule gets a 🔴, and `--fail-on violation` can block the merge.

4. **Blame** (`pryti invariants --blame … --exact`) — find exactly which commit broke a rule:
...
repos. `pryti invariants <repo> --bootstrap --baseline org-baseline.json` discovers the rules,
...
`pryti post … --inline` pins each finding as a comment **on the exact line**. The honest guard: it
...
`pryti post … --label` puts **one** triage label on the PR based on its worst finding:
`pryti:🔴security` / `pryti:🟠payment` / `pryti:🟡new-write` / `pryti:🟢safe-refactor`. Makes a queue
...
Running `pryti review` with **no arguments** inside a repo just does the obvious thing: reviews your
...
Add this workflow and Pryti reviews every PR automatically:

```yaml
# .github/workflows/pryti.yml
name: Pryti Semantic Review
on: pull_request
permissions:
  contents: read
...
| `label` | `true` | Apply one `pryti:*` label. ([#6](#feature-6--auto-label)) |
...
## 10. Set up Pryti for all your repos

Section 9 put Pryti on **one** repo. This section turns it on across **every repo you own** —
personal projects, a team's services, or a whole company — from **one place**, with almost no
per-repo work.

The trick: **host the workflow once, and have every repo point at it.** Then you switch Pryti on (or
...
Make a repo — call it `pryti-workflows` (under your username or org) — and add the real workflow
there **once**, as a **reusable workflow**:

```yaml
# pryti-workflows/.github/workflows/pryti.yml
name: Pryti
on: { workflow_call: {} }
...
```yaml
# any-repo/.github/workflows/pryti.yml
name: Pryti
on: { pull_request: {} }
jobs:
  pryti:
    uses: your-name/pryti-workflows/.github/workflows/pryti.yml@v1
    permissions: { contents: read, pull-requests: write, issues: write }
```

Now updating Pryti *everywhere* means editing **one** file in `pryti-workflows`. That's the
...
Notice `fail_on: none` above. That's **observe mode**: Pryti posts its review on every PR but
...
> Keep `issues: write` + `label: 'true'` on. Pryti puts one `pryti:*` label on each PR, and Step 4's
...
This is the part that makes Pryti *yours*: enforcing the promises **your** code already keeps
...
`pryti invariants --bootstrap` does it with **no human**:

```bash
pryti invariants <repo> --bootstrap --baseline org-baseline.json --out pryti-invariants.json
```
...
each repo's `pryti-invariants.json`, and the workflow enforces it from then on.
...
right now* — `pryti digest` counts the open PRs carrying each `pryti:*` label:

```bash
pryti digest --org your-github-org --slack "$SLACK_WEBHOOK"
```
...
place. And to reassure any security reviewer: Pryti **never runs your code** — it reads it with
...
```bash
pryti review /path/to/repo --commit <sha> --sarif out.sarif
```

Pryti produces, in one pass:
...
Pryti's whole value is that you can **trust** it. So it follows strict rules:

- **Never render "not observed" as "doesn't exist."** If Pryti can't follow something, it shows ⚠ or
...
- **Every number traces to a `file:line`.** Because Pryti reports facts, not vibes.
...
```bash
# review
pryti review                                  # this branch vs default branch
pryti review . --base main --head feature     # two refs
pryti review URL --pr 123                      # a GitHub PR
pryti review . --commit <sha>                  # one commit
pryti review . --sarif out.sarif --html r.html # extra outputs
pryti review . --invariants rules.json --fail-on violation   # enforce + block

# post to a PR
pryti post 123 pr_review.md --json pr_review.json --inline --label
pryti post 123 pr_review.md --dry-run          # preview, no posting

# web UI
pryti serve                                    # this repo
pryti serve --repo . --invariants rules.json   # + enable Confirm buttons

# invariants (rules)
pryti invariants .                             # discover candidate rules
pryti invariants --confirm                     # confirm interactively
pryti invariants --confirm --id auth-before-write --owner security
pryti invariants . --bootstrap --baseline org-baseline.json   # auto-confirm + ratchet (no human)
pryti invariants . --blame auth-before-write --exact   # who broke it

# digest (org-wide roll-up for leadership)
pryti digest --org my-org                       # open PRs by pryti:* label
pryti digest --org my-org --repos repos.txt --slack "$SLACK_WEBHOOK"  # + adoption, to Slack

# env vars
PRYTI_SCAN_DEPS=0     # skip the dependency network scan
PRYTI_TOKEN=…         # lock down a hosted `pryti serve`
```

---

*Pryti is open source. Built by [Ankush Singh Gandhi](https://warriorwhocodes.com). Questions,
stars, and sponsorships welcome on [GitHub](https://github.com/AnkushSinghGandhi/pryti-semantic-reviewer).*
